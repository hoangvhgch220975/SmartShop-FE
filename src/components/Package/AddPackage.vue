<template>
    <div class="add-package container mt-5 mb-5">
      <h2 class="form-title text-success font-weight-bold mb-4 text-center">➕ Add New Package</h2>
  ...
  <script>
  import { addPackage, getAllPackages } from "@/api/PackageAPI";
      ...
      async submitForm() {
        try {
          ...
          const nameExists = existing.data.some(
            (pkg) =>
              pkg.name.trim().toLowerCase() === this.form.name.trim().toLowerCase()
          );
  
          if (nameExists) {
            alert("⚠️ A package with this name already exists.");
            return;
          }
  
          if (!/^\d+(\.\d{1,2})?$/.test(this.form.price)) {
            alert("⚠️ Please enter a valid price (e.g., 99.99)");
            return;
          }
  
          await addPackage(this.form);
          alert("✅ Package added successfully!");
          this.$router.push("/admin/packages");
        } catch (err) {
          alert("❌ Failed to add package: " + (err.message || "Unknown error"));
        }
      },
    },
  };
